...
def listorder(request):#返回一个QuerSet对象，包含所有的表记录
    qs = Order.objects\
        .annotate(
        customer_name = F('cutomer__name'),
        medicines_name = F('medicines__name'))\
        .values('id', 'name', 'create_date', 'customer_name', 'medicines_name')#双下划线查询关联表的字段
...
def addorder(request):
    ...
    with transaction.atomic():# 对应数据库中的事务操作
    ...
    #.create创建一条记录  .bulk_create创建多条记录
